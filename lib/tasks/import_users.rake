...
class UsersRowProcessor
  ...
  attr_reader :row, :stats, :name, :role, :email
  ...
  def upsert_user
    ...
    if role_record.nil?
      puts "❌ Row #{stats[:total]} error: Role '#{role}' does not exist in the roles table."
      stats[:errors] += 1
      return
    end
    ...
    if user
      user.update!(name: name, role: role_record)
      puts "Updated: #{email} (#{name}, #{role_record.name})"
      stats[:updated] += 1
    else
      User.create!(name: name, email: email, role: role_record)
...
class UsersSampleFormatter
  def self.display
    ...
    puts "=" * 80
    puts "name,role,email"
    puts "John Doe,Manager,john@example.com"
    puts "Sarah Tan,Clerk,sarah@example.com"
    puts "Michael Lee,Field Conductor,mlee@example.com"
    puts "=" * 80
